...
function CustomTooltip({ active, payload, label, transactions }) {
  ...
  return (
    <div className="bg-[#0f1629] border border-[#1a2240] rounded-xl p-3 text-xs font-mono min-w-[180px]">
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color }}>{p.name}: {p.value != null ? `$${p.value.toLocaleString("en-US", { minimumFractionDigits: 2 })}` : "—"}</p>
      ))}
      {tx?.length > 0 && (
        <div className="mt-2 border-t border-[#1a2240] pt-2 space-y-1">
          {tx.map((t, i) => (
            <p key={i} className={t.type === "buy" ? "text-[#00ff88]" : "text-[#ff4757]"}>
              {t.type === "buy" ? "📥" : "📤"} {t.symbol} — {t.type === "sell" && t.realized_pnl != null ? `P&L: $${fmt2(t.realized_pnl)}` : `$${fmt2(t.total_amount)}`}
            </p>
          ))}
        </div>
      )}
...
export default function Performance() {
  ...
  useEffect(() => {
    async function load() {
      const [snaps, txs, configs, spyRecs] = await Promise.all([
        base44.entities.PerformanceSnapshot.list("-timestamp", 200),
        base44.entities.Transaction.list("-executed_at", 200),
        base44.entities.UserConfig.list("-created_date", 1),
        base44.entities.SP500History.list("-timestamp", 200)
      ]);
      setSnapshots(snaps || []);
      setTransactions(txs || []);
  ...
  const filteredSnaps = snapshots.filter(s => new Date(s.timestamp).getTime() >= rangeCutoff);
  ...
  const spyByDay = {};
  spy.filter(s => new Date(s.timestamp).getTime() >= rangeCutoff).forEach(s => {
    const d = new Date(s.timestamp).toLocaleDateString("en-US");
  ...
  const basePortfolio = days.length > 0 ? byDay[days[0]].value : 1;
  const baseSpy = days.length > 0 && spyByDay[days[0]] ? spyByDay[days[0]] : 1;
  const goalAmount = config?.goal_amount || null;

  const chartData = days.map(d => ({
    date: d,
    portfolio: byDay[d].value,
    spy: spyByDay[d] != null ? spyByDay[d] : null,
    goal: goalAmount
  }));

  // KPIs
  const firstVal = chartData[0]?.portfolio;
  const lastVal = chartData[chartData.length - 1]?.portfolio;
  const myReturn = firstVal ? ((lastVal - firstVal) / firstVal) * 100 : 0;

  const firstSpy = chartData[0]?.spy;
  const lastSpy = chartData[chartData.length - 1]?.spy;
  const spyReturn = firstSpy ? ((lastSpy - firstSpy) / firstSpy) * 100 : 0;
  ...
  return (
    <div>
        ...
        {loading || chartData.length < 2 ? (
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={chartData}>
              ...
              <Line type="monotone" dataKey="portfolio" name="Mi Portafolio" stroke="#00ff88" strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="spy" name="S&P 500 (SPY)" stroke="#64748b" strokeWidth={1.5} dot={false} strokeDasharray="none" />
              {goalAmount && (
                <Line type="monotone" dataKey="goal" name="Mi Meta" stroke="#3b82f6" strokeWidth={1.5} dot={false} strokeDasharray="6 3" />
              )}
            </LineChart>
          </ResponsiveContainer>
        )}
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Mi Retorno", value: `${myReturn >= 0 ? "+" : ""}${myReturn.toFixed(2)}%`, color: myReturn >= 0 ? "#00ff88" : "#ff4757" },
          { label: "S&P 500", value: `${spyReturn >= 0 ? "+" : ""}${spyReturn.toFixed(2)}%`, color: "#64748b" },
          { label: "Alfa vs Mercado", value: `${diff >= 0 ? "+" : ""}${diff.toFixed(2)}%`, color: diff >= 0 ? "#00ff88" : "#ff4757" }
        ].map(({ label, value, color }) => (
          <motion.div key={label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card-terminal p-4 text-center">
